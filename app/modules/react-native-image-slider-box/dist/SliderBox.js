...
import {View, Image, ActivityIndicator, TouchableOpacity, Dimensions} from 'react-native';
...
export class SliderBox extends Component {
  ...
  _renderItem({item, index}) {
    ...
    return (
      <View
        ...
        <TouchableOpacity key={index} onPress={() => !disableOnPress && this.onCurrentImagePressedHandler()}>
          <ImageComponent
            style={[
              {
                width: '100%',
  ...
  render() {
    const {images, circleLoop, autoplay, parentWidth, loopClonesPerSide} = this.props;
    return (
      <View>
        <Carousel
          layout={'default'}
