...
export default class ActionSheet extends Component {
  constructor(props) {
    ...
    safeareaHeight = safeareaHeight || getDeviceHeight(this.props.statusBarTranslucent);
    innerViewHeight = innerViewHeight || getDeviceHeight(this.props.statusBarTranslucent);
  ...
  _hideAnimation() {
    let {animated, closeAnimationDuration, onClose, bottomOffset, initialOffsetFromBottom, extraScroll, closable} = this.props;
    ...
    waitAsync(closeAnimationDuration / 1.5).then(() => {
      let scrollOffset = closable
        ? 0
        : this.customComponentHeight * initialOffsetFromBottom + this.state.deviceHeight * 0.1 + extraScroll - bottomOffset;
  ...
  _showModal = async (event) => {
    let {gestureEnabled, initialOffsetFromBottom, extraScroll, delayActionSheetDraw, delayActionSheetDrawTime} = this.props;
    ...
    if (this.layoutHasCalled) {
      ...
      let scrollOffset = gestureEnabled
        ? this.customComponentHeight * initialOffsetFromBottom + correction + extraScroll
  ...
  _applyHeightLimiter() {
    if (this.customComponentHeight > this.state.deviceHeight) {
      this.customComponentHeight = (this.customComponentHeight - (this.customComponentHeight - this.state.deviceHeight)) * 1;
    }
  }
  ...
  _onScrollEnd = async (event) => {
    ...
    let correction = this.state.deviceHeight * 0.1;
    let distanceFromTop = this.customComponentHeight + correction - this.offsetY;
  ...
  _returnToPrevScrollPosition(height) {
    let offset = height * this.currentOffsetFromBottom + this.state.deviceHeight * 0.1 + this.props.extraScroll;
    this._scrollTo(offset);
  }
  ...
  _onDeviceLayout = (event) => {
    ...
    this.layoutTime = setTimeout(() => {
      ...
      if (height?.toFixed(0) === calculatedDeviceHeight?.toFixed(0) && width?.toFixed(0) === this.state.deviceWidth?.toFixed(0))
        return;
                  ...
                  {!hideUnderlay && (
                    <Animated.View
                      style={{
                        ...
                        borderTopLeftRadius: containerStyle?.borderTopLeftRadius || 10,
                        borderTopRightRadius: containerStyle?.borderTopRightRadius || 10,
                        transform: [
                          {
