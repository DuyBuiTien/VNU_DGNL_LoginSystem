...
const BDHC_UnitChildScreen = () => {
  ...
  let HOST_YT = dataService.HOST_YT;
  ...
  data1.map((i) => {
    var url = i.split(',')[0];
    dataAnh.push(url);
    dataUrl.push({url: `${dataService.HOST_YT}${url}`});
  });

  const _renderScrollViewContent = (dataInfo_) => {
    var dt = dataInfo.TuNhien_DiaHinh_DienTich ? `${dataInfo.TuNhien_DiaHinh_DienTich}` : 'Không có số liệu';
    var ds = dataInfo.TongQuan_DanSo_Tong ? `${dataInfo.TongQuan_DanSo_Tong}` : 'Không có số liệu';
    ...
    return (
      <View style={{padding: 15, paddingTop: top}}>
        ...
        <Text style={{fontSize: 18, paddingTop: 30, paddingBottom: 10, fontWeight: '600'}}>Ảnh</Text>
        <TouchableOpacity
  ...
  console.log('urlAnh1urlAnh1urlAnh1');
  console.log(urlAnh1);
    ...
    <View style={styles.container}>
      <View style={{flex: 1}}>
        <Animated.ScrollView
          ...
          {_renderScrollViewContent(dataInfo)}
        </Animated.ScrollView>
