...
const RenderItem = (props) => {
  ...
  return (
    <TouchableOpacity
      onPress={() => this.setState({visibleModal: true, detail: data})}
      style={{
        padding: 10,
        flexDirection: 'row',
        borderRadius: 5,
...
const MainScreen = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    const res = await requestGET(`${dataService.YT_URL}/DiaChiYTes/mGetListQT`);
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
      <Header title="Quầy thuốc" isStack={true} />
