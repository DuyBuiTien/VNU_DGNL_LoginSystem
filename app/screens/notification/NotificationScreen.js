...
/*Data*/
import artistData from './SongData.json';
const coverImage = require('./assets/background.jpeg');
const profileImage = require('./assets/background.jpeg');
...
export default class ArtistScreen extends Component {
  ...
  renderArtistCard = (index, item) => {
    return (
      <View key={index.toString()} index={index}>
        <TouchableOpacity
          activeOpacity={0.8}
  ...
  _getImageLeftPosition = () => {
    ...
    return scrollY.interpolate({
      inputRange: [0, 140],
      outputRange: [ThemeUtils.relativeWidth(30), ThemeUtils.relativeWidth(10)],
      extrapolate: 'clamp',
      useNativeDriver: true,
    });
  };
  ...
  _getImageTopPosition = () => {
    ...
    return scrollY.interpolate({
      inputRange: [0, 140],
      outputRange: [ThemeUtils.relativeHeight(20), 20],
      extrapolate: 'clamp',
      useNativeDriver: true,
    });
  };
